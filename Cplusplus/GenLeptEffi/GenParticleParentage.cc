...
GenParticleParentage(reco::GenParticleRef& _match) {
  if( _match.isNonnull() && _match.isAvailable() ) {
	//std::cout<<"Whennotwhat:"<<_match->pdgId()<<std::endl;
...
void GenParticleParentage::getParentageRecursive(const reco::GenParticleRef& p, int daughterId) {
  ...
  //if( p->pt() < 0.1 ) return;  // reached initial state quarks/proton
	...
	//std::cout<<"Whennotwhat2:"<<p->pdgId()<<std::endl;
  ...
  if ( std::abs(daughterId) != std::abs(p->pdgId()) && daughterId != 0 ) {    
	//std::cout<<"Whennotwhat3:"<<p->pdgId()<<std::endl;
	...
	//std::cout<<"not Neutrinos or photons:"<<p->pdgId()<<std::endl;
    case 11:
    case 13:
    case 15:
    ...
    case 6:
    case 2212:
	//std::cout<<"not Neutrinos or photons or leptons:"<<p->pdgId()<<std::endl;
      _qcdParents.push_back(p);
      break;
    case 23:
    case 24:
    case 25:
	//std::cout<<"not Neutrinos or photons or leptons or quarks:"<<p->pdgId()<<std::endl;
