...
void MSSigstackedRegion(TString region, Bool_t dolog, Bool_t HIP, Bool_t useEOS, TString description)
{
...
//     }
     ...
     if(drawSignal){
       ...
       sigleg->AddEntry(h_Sig_MS7ct1    , "Sig(H#rightarrow SS#rightarrow dddd): M_{S}=7 c#tau_{S}=1   ", "l" ) ;
       sigleg->AddEntry(h_Sig_MS7ct10    , "Sig(H#rightarrow SS#rightarrow dddd): M_{S}=7 c#tau_{S}=10   ", "l" ) ;
       sigleg->AddEntry(h_Sig_MS7ct100    , "Sig(H#rightarrow SS#rightarrow dddd): M_{S}=7 c#tau_{S}=100   ", "l" ) ;
       sigleg->AddEntry(h_Sig_MS7ct1000    , "Sig(H#rightarrow SS#rightarrow dddd): M_{S}=7 c#tau_{S}=1000   ", "l" ) ;
     }
     // TLine
     ...
     if(drawSignal){
       h_Sig_MS7ct1->SetLineColor(kGreen);
       h_Sig_MS7ct1->SetLineWidth(4);
       h_Sig_MS7ct1->SetLineStyle(1);
       h_Sig_MS7ct1->Scale(1/h_Sig_MS7ct1->Integral(0,-1));
       h_Sig_MS7ct1->Draw("hist sames") ;
       h_Sig_MS7ct10->SetLineColor(kOrange);
       ...
       h_Sig_MS7ct100->SetLineColor(kViolet);
       h_Sig_MS7ct100->SetLineWidth(4);
       h_Sig_MS7ct100->SetLineStyle(1);
       h_Sig_MS7ct100->Scale(1/h_Sig_MS7ct100->Integral(0,-1));
       h_Sig_MS7ct100->Draw("hist sames") ;
       h_Sig_MS7ct1000->SetLineColor(kBlue);
       h_Sig_MS7ct1000->SetLineWidth(4);
       h_Sig_MS7ct1000->SetLineStyle(1);
       h_Sig_MS7ct1000->Scale(1/h_Sig_MS7ct1000->Integral(0,-1));
       h_Sig_MS7ct1000->Draw("hist sames") ;
       sigleg->Draw();
     }
     leg->Draw();
   ...
   //extra->DrawLatexNDC(0.48,0.78,"#rightarrow");
     ...
     // save canvas
     canvas->SaveAs(outname+description+"TVPlot2SigMS7"+".png");
     canvas->SaveAs(outname+description+"TVPlot2SigMS7"+".pdf");
  
     // save histograms into single root file
     TFile *outfile = TFile::Open(outname+"TVPlot2SigMS7"+".root","RECREATE");
     h_Data        ->Write();
     h_DY          ->Write();
     h_GJets       ->Write();
     h_ST          ->Write();
