...
void plotter_Signal_stackedRegion(TString region, Bool_t dolog, Bool_t HIP, Bool_t doctau, Bool_t useEOS, TString description)
{
...
cout<<"where"<<endl;
 ...
 file_ZH_NoUnc                    = TFile::Open( inpath +region+"_"+variable+"_log"+uncbin+"_"+MSsample+".root"                 ) ;
    ...
    if(j==0){
     ...
     h_Sig_NoUnc       = (TH1F*)file_ZH_NoUnc    ->Get("Sig_NoUnc" )->Clone( "Sig_NoUnc" ) ;
     h_Sig_JECUp       = (TH1F*)file_ZH_NoUnc    ->Get("Sig_JECUp" )->Clone( "Sig_JECUp" ) ;
     h_Sig_JECDown       = (TH1F*)file_ZH_NoUnc    ->Get("Sig_JECDown" )->Clone( "Sig_JECDown" ) ;
	...
	if(uncbin == "_JECUp"){
       ...
       sigleg->Draw();
TPaveText *fitbox = new TPaveText(.45,.3,.9,.6,"brNDC");
fitbox->SetBorderSize(1);
fitbox->SetFillColor(0);
fitbox->SetTextSize(0.03);
fitbox->AddText(Form("#int NoUnc = %g, %g",h_Sig_NoUnc->Integral(),1.));
fitbox->AddText(Form("#int JECUp = %g, %g",h_Sig_JECUp->Integral(), h_Sig_JECUp->Integral()/h_Sig_NoUnc->Integral()));
fitbox->AddText(Form("#int JECDown = %g, %g",h_Sig_JECDown->Integral(), h_Sig_JECDown->Integral()/h_Sig_NoUnc->Integral()));
fitbox->Draw();
     char lumistring [50];
     int dummy; 
