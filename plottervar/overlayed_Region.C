...
void overlayed_Region(TString region, Bool_t dolog, Bool_t HIP, Bool_t doctau, Bool_t useEOS, TString description)
{
...
// Bool_t drawRatio = kTRUE;
 ...
 TString inpath_JECUp = inpath+"New_JECUp_35GeV/";
 TString inpath_JECDown = inpath+"New_JECDown_35GeV/";
...
variables.push_back("AllJets_0Tag_AODCaloJetPt");
variables.push_back("AllJets_1Tag_AODCaloJetPt");
variables.push_back("AllJets_2Tag_AODCaloJetPt");
  ...
  for(unsigned int j=0; j<variables.size(); ++j){
   ...
   for(unsigned int tt=0; tt<loopEnd; ++tt){
    ...
    if(j==0){
     ...
     h_Sig_NoUnc       = (TH1F*)file_ZH_NoUnc    ->Get("h_"+varname )->Clone( "Sig_NoUnc" ) ;
     h_Sig_JECUp       = (TH1F*)file_ZH_JECUp    ->Get("h_"+varname )->Clone( "Sig_JECUp" ) ;
     h_Sig_JECDown       = (TH1F*)file_ZH_JECDown    ->Get("h_"+varname )->Clone( "Sig_JECDown" ) ;

     h_Sig_NoUnc  ->Scale(MCSF);
     h_Sig_JECUp  ->Scale(MCSF);
     h_Sig_JECDown  ->Scale(MCSF);
	std::cout<<"NoUnc:"<<h_Sig_NoUnc->Integral()<<std::endl;
	std::cout<<"JECUp:"<<h_Sig_JECUp->Integral()<<std::endl;
	std::cout<<"JECDown:"<<h_Sig_JECDown->Integral()<<std::endl;
	...
	if(uncbin=="_JECUp") sigleg->AddEntry(h_Sig_JECDown   , "Sig JECDown", "l" ) ;
     // set max and draw
     double_t ymax;
     ymax = std::max(h_Sig_JECUp->GetMaximum(), h_Sig_JECDown->GetMaximum() );
     if(dolog){
    ...
    if(MSsample.Contains("HToSSTobbbb") &&variable=="nSelectedAODCaloJetTag" ) 
	{
